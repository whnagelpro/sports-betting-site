...
exports.handler = async function (event) {
  ...
  if (!sourceUrl) {
    return {
      ...
      body: JSON.stringify({
        error: "Missing NHL_PROPS_SOURCE_URL environment variable"
      })
    };
  }
  ...
  try {
    ...
    if (!ALLOWED_TIERS.includes(profile.tier)) {
      return {
        ...
        body: JSON.stringify({
          ...
          message: "NHL premium props require All-Star or higher",
          tier: profile.tier
        })
      };
    }
    ...
    if (!response.ok) {
      return {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          error: `Failed to fetch NHL props CSV: ${response.status}`
        })
      };
    }
    ...
    return {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        error: "Server error while fetching protected NHL props",
